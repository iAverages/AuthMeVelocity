...
import fr.xephi.authmebungee.common.utils.ExceptionUtils;
 ...
 * AuthMeBungee logger.
 */
public final class ConsoleLogger implements GenericLogger {
     ...
     */
    public static void initializeSharedSettings(SettingsManager settings) {
        boolean useLogging = settings.getProperty(ProxyConfigProperties.USE_LOGGING);
        if (useLogging) {
     ...
     * @param settings the settings to read from
     */
    public void initializeSettings(SettingsManager settings) {
    ...
    private static void initializeFileWriter() {
        if (fileWriter == null) {
            ...
            try {
                ...
                closeSafely(fos);
                logger.error("Failed to create writer to AuthMe log file", e);
            }
        }
    }
}
